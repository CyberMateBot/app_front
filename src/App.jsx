...
const translations = {
    ru: {
        ...
        modelKlingO3StdName: 'Kling Video O3 Standard',
        modelKlingO3StdSub: 'Кинематографичное видео 720p со звуком',
        modelKlingO3ProName: 'Kling Video O3 Pro',
        modelKlingO3ProSub: 'Ультра-реалистичное видео 1080p со звуком',
        modelKlingO34kName: 'Kling Video O3 4K',
        modelKlingO34kSub: 'Сверхвысокая четкость 4K со звуком',
        modelKling3TurboStdName: 'Kling 3.0 Turbo Std',
        modelKling3TurboStdSub: 'Быстрая генерация Kling 3.0 720p',
        modelKling3TurboProName: 'Kling 3.0 Turbo Pro',
        modelKling3TurboProSub: 'Скоростная генерация Kling 3.0 1080p',
        modelKling26StdName: 'Kling 2.6 Standard',
        modelKling26StdSub: 'Плавное движение и четкость Kling 2.6',
        modelKling26ProName: 'Kling 2.6 Pro',
        modelKling26ProSub: 'Высокое качество и нативный звук Kling 2.6',
        modelKling21MasterName: 'Kling 2.1 Master',
        modelKling21MasterSub: 'Кинематографичная динамика Kling 2.1',
        modelKling20MasterName: 'Kling 2.0 Master',
        modelKling20MasterSub: 'Классический генератор Kling 2.0',
        modelKling16StdName: 'Kling 1.6 Standard',
        modelKling16StdSub: 'Проверенная генерация Kling 1.6 Standard',
        modelKling16ProName: 'Kling 1.6 Pro I2V',
        modelKling16ProSub: 'Анимация фото в видео Kling 1.6 Pro',
        modelKlingVideoO1Name: 'Kling Video O1',
        modelKlingVideoO1Sub: 'Omni видеомодель Kling Video O1',
        modelKlingImageGroupName: 'Kling Image',
        modelKlingImageGroupSub: 'Генерация и редактирование Kling Image',
        modelKlingImageO3Name: 'Kling Image O3',
        modelKlingImageO3Sub: 'Флагманская Kling Image O3 до 4K',
        modelKlingImageV3Name: 'Kling Image 3.0',
        modelKlingImageV3Sub: 'Высокодетализированная Kling Image 3.0',
        modelKlingV1TtsName: 'Kling V1 TTS',
        modelKlingV1TtsSub: 'Реалистичный синтез речи Kling V1',
        ...
        infoFaqA5: 'Нет, часть моделей доступна на бесплатном плане. Платная подписка открывает доступ к более мощным и продвинутым моделям и начисляет на баланс пакет CyberCoins при покупке. Сама стоимость генерации в CyberCoins одинакова для всех пользователей и не зависит от подписки.',
    ...
    en: {
        ...
        modelKlingO3StdName: 'Kling Video O3 Standard',
        modelKlingO3StdSub: 'Cinematic 720p video with native audio',
        modelKlingO3ProName: 'Kling Video O3 Pro',
        modelKlingO3ProSub: 'Ultra-realistic 1080p video with audio',
        modelKlingO34kName: 'Kling Video O3 4K',
        modelKlingO34kSub: 'Ultra-high definition 4K with audio',
        modelKling3TurboStdName: 'Kling 3.0 Turbo Std',
        modelKling3TurboStdSub: 'Fast Kling 3.0 720p generation',
        modelKling3TurboProName: 'Kling 3.0 Turbo Pro',
        modelKling3TurboProSub: 'Fast Kling 3.0 1080p generation',
        modelKling26StdName: 'Kling 2.6 Standard',
        modelKling26StdSub: 'Smooth motion and clarity Kling 2.6',
        modelKling26ProName: 'Kling 2.6 Pro',
        modelKling26ProSub: 'High visual quality and native audio',
        modelKling21MasterName: 'Kling 2.1 Master',
        modelKling21MasterSub: 'Cinematic dynamics Kling 2.1 Master',
        modelKling20MasterName: 'Kling 2.0 Master',
        modelKling20MasterSub: 'Classic Kling 2.0 video generator',
        modelKling16StdName: 'Kling 1.6 Standard',
        modelKling16StdSub: 'Proven Kling 1.6 standard generation',
        modelKling16ProName: 'Kling 1.6 Pro I2V',
        modelKling16ProSub: 'Photo-to-video animation Kling 1.6 Pro',
        modelKlingVideoO1Name: 'Kling Video O1',
        modelKlingVideoO1Sub: 'Omni video model Kling Video O1',
        modelKlingImageGroupName: 'Kling Image',
        modelKlingImageGroupSub: 'Kling Image generation and editing',
        modelKlingImageO3Name: 'Kling Image O3',
        modelKlingImageO3Sub: 'Flagship Kling Image O3 up to 4K',
        modelKlingImageV3Name: 'Kling Image 3.0',
        modelKlingImageV3Sub: 'High-detail Kling Image 3.0',
        modelKlingV1TtsName: 'Kling V1 TTS',
        modelKlingV1TtsSub: 'Realistic speech synthesis Kling V1',
        modelSeedanceGroupName: 'Seedance',
        ...
        mediaModelPhotoBadge: 'From photo',
        ...
        infoFaqA5: 'No, some models are available on the free plan. A paid subscription unlocks access to more powerful models and credits a bundle of CyberCoins to your balance when you buy it. The CyberCoin price of each generation itself is the same for every user, regardless of plan.',
...
function App() {
    ...
    // Measure the real rendered height of the bottom nav (varies by device
    // safe-area, font scaling, theme) so every page reserves exactly enough
    // bottom padding — no content hidden behind the floating nav bar.
    useLayoutEffect(() => {
        const navEl = bottomNavRef.current;

        if (!showBottomNav || !navEl) {
            return undefined;
        }
        ...
        const updateNavHeight = () => {
            ...
            if (height > 0) {
                document.documentElement.style.setProperty('--nav-bar-height', `${Math.ceil(height)}px`);
            }
        };
    ...
    // Peek carousel: cards are ~68% of the scroll viewport so both
    // neighbors stay visible. Touch swipe on mobile; mouse drag on
    // desktop (wheel / trackpad scrolling is blocked). The `--active`
    // class follows whichever card is closest to center so CSS can
    // enlarge it relative to its neighbors.
    useEffect(() => {
        ...
        // Layout offsets only — getBoundingClientRect includes CSS `scale`
        // and the intro `transform` animation, which threw the previous
        // center math off (visual left + layout width/2). Walk offsetParent
        // up to the scroll container (it is `position: relative`).
        const layoutLeftWithin = (container, child) => {
            let left = 0;
            let node = child;
        ...
        const cardLayoutCenter = (card) => layoutLeftWithin(el, card) + card.offsetWidth / 2;

        const scrollTargetForCard = (card) => {
            const maxScrollLeft = Math.max(0, el.scrollWidth - el.clientWidth);
            return Math.max(0, Math.min(
                maxScrollLeft,
                cardLayoutCenter(card) - el.clientWidth / 2,
            ));
        };
        ...
        const findClosestCard = () => {
            ...
            cards.forEach((card) => {
                const dist = Math.abs(cardLayoutCenter(card) - viewportCenter);
                if (dist < closestDist) {
                    closestDist = dist;
                    closest = card;
                }
        ...
        const updateActive = () => {
            ...
            cards.forEach((card) => {
                card.classList.toggle('subscription-plan-card--active', card === active);
            });
        };
        ...
        const syncMetrics = () => {
            ...
            el.style.setProperty('--sub-card-w', `${(width * CARD_RATIO).toFixed(1)}px`);
            el.style.setProperty('--sub-side-pad', `${((1 - CARD_RATIO) * width / 2).toFixed(1)}px`);
        };
        ...
        const centerCurrentPlan = () => {
            ...
            el.scrollTo({ left: scrollTargetForCard(currentCard) });
        ...
        // Two rAFs: first applies the px metrics, second centers after
        // the flex row has laid out at the new card width. ResizeObserver
        // covers Telegram webview size changes after that.
        centerRafId = window.requestAnimationFrame(() => {
            syncMetrics();
            centerRafId = window.requestAnimationFrame(centerCurrentPlan);
        });
        ...
        return () => {
            ...
            delete el.dataset.dragging;
            el.style.removeProperty('--sub-card-w');
            el.style.removeProperty('--sub-side-pad');
        };
    ...
    return (
        ...
        >
            ...
            {showBottomNav ? (
                <nav ref={bottomNavRef} className="bottom-nav bottom-nav--concept" aria-label={language === 'ru' ? 'Основная навигация' : 'Main navigation'}>
